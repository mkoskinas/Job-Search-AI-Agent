"""
CV Analyzer Tool Module

This module provides functionality for analyzing CVs/resumes against job descriptions
using a combination of NLP techniques, semantic similarity, and LLM-based analysis.
Features include:
...
class CVAnalyzerInput(BaseModel):
    """
    Input schema for CV analysis.

    Attributes:
        cv_text (str): The text content of the CV/resume to analyze
...
class CVAnalyzerTool(BaseTool):
    """
    A tool for analysing CVs/resumes against job descriptions.

    This tool performs detailed analysis using:
    - Keyword extraction and matching
    - Semantic similarity scoring
    - LLM-based verification
    - ATS compatibility checking

    The analysis includes scores for:
    ...
    embeddings: OpenAIEmbeddings = Field(
        default_factory=lambda: OpenAIEmbeddings(chunk_size=1000, request_timeout=30)
    )
    nlp: Any = Field(default_factory=lambda: spacy.load("en_core_web_sm"))
    llm: ChatOpenAI = Field(
        default_factory=lambda: ChatOpenAI(
            model="gpt-4-turbo-preview", temperature=0, request_timeout=60
        )
    )
    ...
    def extract_keywords(
        self, text: str, pos: List[str] = ["NOUN", "PROPN", "ADJ"]
    ) -> List[str]:
        """
        ...
        Args:
            ...
            pos (List[str] | None): Parts of speech to extract. Defaults to ["NOUN", "PROPN", "ADJ"]
        ...
        for chunk in doc.noun_chunks:
            ...
            if (
                ...
                and not any(word in chunk.text.lower() for word in blacklist)
            ):
    ...
    def _get_structured_analysis(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        required_matches = sum(
            ...
            if any(self._is_similar_keyword(k, cv_k) for cv_k in cv_keywords)
        )
        ...
        if required_keywords:
            ...
            preferred_score = (
                (preferred_matches / len(preferred_keywords)) * 30
        ...
        try:
            ...
            client = chromadb.EphemeralClient(
                Settings(
                    anonymized_telemetry=False, allow_reset=True, is_persistent=False
                )
            )

            # Create vectorstore using the ephemeral client
            vectorstore = Chroma.from_texts(
                texts=[cv_text], embedding=self.embeddings, client=client
            )

            # Add CV text and search against job description
            results = vectorstore.similarity_search_with_relevance_scores(job_desc, k=1)
        ...
        return {
            "structured_score": round((keyword_score * 0.3 + semantic_score * 0.7), 2),
        ...
        try:
            ...
            try:
                ...
                final_score = (
                    ...
                    + llm_analysis["category_scores"]["keyword_optimization"]["score"]
                    * 0.20
                ...
                self.logger.info(
                    f"LLM verification took {time() - start_time:.2f} seconds"
                )
                return {
    ...
    def _blend_scores(self, structured_score: float, llm_score: float) -> float:
        ...
        return round(structured_score * 0.7 + llm_score * 0.3, 2)
    ...
    def _get_top_strengths(self, result: Dict[str, Any]) -> str:
            ...
            [
                {
                    ...
                    "score": calculated_scores["technical_fit"],
                },
                {"description": "Role Match", "score": calculated_scores["role_match"]},
                {
                    "description": "Keyword Match",
                    "score": calculated_scores["keyword_match_score"],
                },
        ...
        metrics_map = {
            ...
            "formatting": ["clarity", "section_flow"],
            "brevity": ["impact", "redundancy"],
        }
    ...
    def _get_areas_for_focus(self, result: Dict[str, Any]) -> str:
        ...
        metrics_map = {
            ...
            "formatting": ["ats_compatibility", "clarity", "conciseness"],
            "achievements": [
                "business_impact",
                "leadership_collaboration",
                "innovation",
